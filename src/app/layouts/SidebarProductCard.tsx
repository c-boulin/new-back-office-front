...
export function SidebarProductCard() {
  ...
  const onSelect = (tenantId: string) => {
    ...
    setOpen(false);
    const currentSlug = active?.tenantSlug;
    const subPath = currentSlug
      ? location.pathname.replace(new RegExp(`^/t/${currentSlug}/?`), "")
      : "";
    navigate(`/t/${next.tenantSlug}${subPath ? `/${subPath}` : ""}`);
  };
