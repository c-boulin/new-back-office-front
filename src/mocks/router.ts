...
const routes: Array<{ method: string; pattern: string; handler: Route }> = [
  ...
  { method: "PUT", pattern: "/v1/product-config", handler: (r) => ok(productConfig.update(tenantHeader(r), r.body)) },
  { method: "DELETE", pattern: "/v1/product-config", handler: (r) => { productConfig.remove(tenantHeader(r)); return { status: 204, data: null }; } },
