...
describe("boUserFromRaw", () => {
  ...
  it("passes through a valid hex color as-is", () => {
    const user = boUserFromRaw({
      ...raw,
      products: [{ id: 10, name: "Woozgo", slug: null, role: { id: 5, color: "#ff0000" } }],
    });
    expect(user.products[0].role.color).toBe("#ff0000");
  });

  it("maps a legacy keyword color to hex", () => {
    expect(boUserFromRaw(raw).products[0].role.color).toBe("#3b82f6");
  });
