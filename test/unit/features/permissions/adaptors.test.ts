...
describe("roleFromRaw", () => {
  ...
  it("maps legacy keyword colors to hex", () => {
    expect(roleFromRaw({ ...raw, color: "info" }).color).toBe("#3b82f6");
    expect(roleFromRaw({ ...raw, color: "error" }).color).toBe("#ef4444");
  });

  it("passes through valid hex colors as-is", () => {
    expect(roleFromRaw({ ...raw, color: "#abcdef" }).color).toBe("#abcdef");
  });

  it("falls back to default color when the value is unknown", () => {
    expect(roleFromRaw({ ...raw, color: "teal" }).color).toBe("#3b82f6");
  });
