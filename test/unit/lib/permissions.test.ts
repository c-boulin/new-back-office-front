...
describe("hasPermission", () => {
  ...
  it("recognises SETTINGS_CREATE as a distinct permission", () => {
    expect(
      hasPermission([PERMISSIONS.SETTINGS_CREATE], PERMISSIONS.SETTINGS_CREATE),
    ).toBe(true);
    expect(
      hasPermission([PERMISSIONS.SETTINGS_WRITE], PERMISSIONS.SETTINGS_CREATE),
    ).toBe(false);
  });
